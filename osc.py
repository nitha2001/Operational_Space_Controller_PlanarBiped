...
class OperationalSpaceWalkingController(LeafSystem):
    ...
    def CalculateContactJacobian(self, fsm: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        track_point = self.contact_points[fsm]
        J = self.plant.CalcJacobianTranslationalVelocity(self.plant_context, JacobianWrtVariable.kV, track_point.frame,track_point.pt, self.plant.world_frame(), self.plant.world_frame())

        JdotV = self.plant.CalcBiasTranslationalAcceleration(self.plant_context, JacobianWrtVariable.kV, track_point.frame,track_point.pt, self.plant.world_frame(), self.plant.world_frame()).ravel()        
    ...
    def SetupAndSolveQP(self,  context: Context) -> Tuple[np.ndarray, MathematicalProgram]:
        ...
        for traj_name in self.trajs:
            ...
            cost = (yddot_cmd_i - yddot).T @ W_i @ (yddot_cmd_i - yddot)
            prog.AddQuadraticCost(cost)
            

        # TODO: Add Quadratic Cost on vdot using self.gains.w_vdot
        w_vdot = np.eye(7) * self.gains.w_vdot/2
        cost_vdot = vdot.T @ w_vdot @ vdot
        prog.AddQuadraticCost(cost_vdot)
        ...
        expr = M @ vdot + Cv + G - B @ u - J_c.T @ lambda_c
        val = np.zeros(7)
        prog.AddLinearEqualityConstraint(expr,val)

        # TODO: Add Contact Constraint
        expr1 =J_c @ vdot + J_c_dot_v
        val1 = np.zeros(3)
        prog.AddLinearEqualityConstraint(expr1,val1)


        # TODO: Add Friction Cone Constraint assuming mu = 1
        expr2 = np.array([[1, 0, -1], [-1, 0, -1]]) @ lambda_c
        prog.AddLinearConstraint(expr2[0] <= 0)
        prog.AddLinearConstraint(expr2[1] <= 0)
